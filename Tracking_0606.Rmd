...
Go to **Studies**>Search>Write the name of the study

```{r echo=FALSE, out.height=200}
knitr::include_graphics("https://github.com/MiriamLL/R_intro/blob/master/Images/402studies.png?raw=true")
```

Click on the i inside a square>Select Open in studies page

```{r echo=FALSE, out.height=200}
knitr::include_graphics("https://github.com/MiriamLL/R_intro/blob/master/Images/402studies.png?raw=true")
```

Click on the Download>Select Download Data
...
You can download the data here, but they you will need to open it every time.

```{r echo=FALSE, out.height=200}
knitr::include_graphics("https://github.com/MiriamLL/R_intro/blob/master/Images/406Save.png?raw=true")
```

I close it and I dont need to do the permits again, I can download the data directly to R everytime without the problem showing up again. 
...
It would likely show a warning about the sensor, but it did work, you should have **my_individual_track** in the environment now.
...
name: wrangling
class: title-slide, inverse, bottom
background-image: url(https://images.unsplash.com/photo-1569387016077-f46d35a9afb3?ixlib=rb-4.0.3&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D&auto=format&fit=crop&w=687&q=80)
background-size: cover
...
You can expect to repeat the cleaning several time.
...
Base_map<-ggplot(data = Europe)+ geom_sf(color = "#e76f51",fill = "#e9c46a")+ 
  ...
  theme_bw()+
  theme(panel.background = element_rect(fill = '#a8dadc'),panel.grid.major = element_blank(),panel.grid.minor = element_blank())+
  xlab('Longitude')+ylab('Latitude')+
  scale_x_continuous(labels = function(x) paste0(x, '\u00B0', "W"))+ scale_y_continuous(labels = function(x) paste0(x, '\u00B0', "N"))
Base_map
```

See more in my [page](https://www.miriam-lerma.com/posts/2023-03-04-mapping-in-r/)
...
```{r, fig.height=4}
Base_map + 
  geom_path(data=my_individual_subsampled, 
...
I use the function **ggplotly** and then add my map to be able to zoom-in. 
...
I select different **coord_sf** to zoom-in. I repeat the process until I am satisfied.
