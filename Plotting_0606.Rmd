...
The scatterplot is most useful for displaying the relationship between two continuous variables.<br>
Each dot represents an observation. <br>
...
However, most likely we will like to visualize differences between groups.
...
A very recommended book is: [Geocomputation with R](https://r.geocompx.org/attr.html)
...
To plot **sf** or simple features objects, the argument **geom_sf** is to be used
...
To zoom-in we can use the argument **coord_sf**
...
To check which CRS I am using use:

```{r}
German_land$epsg
```
Based on this CRS you can zoom-in
```{r}
ggplot(German_land)+ 
  geom_sf()+
